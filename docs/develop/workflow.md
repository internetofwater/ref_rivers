...
The registry is rebuilt by a [`{targets}`](https://books.ropensci.org/targets/) pipeline in R. `_targets.R` holds the workflow definition and `/R` holds the functions it calls. Most consumers never need to run this — the outputs are published in the data release — but the pipeline is the specification of how mainstems are derived.
...
A new base network is a major version. The work is a new `get_*()` target for the source, extension of `initialize_mainstems()` to reconcile against it, new head and outlet attribute pairs in the published schema, a new provider row, and a new crosswalk in `write_lookups()`. Existing head and outlet references stay where they are.
